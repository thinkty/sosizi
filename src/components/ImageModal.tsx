...
const placeHolderButton: JSX.Element = <div style={{ width: 50, height: 50 }}/>;
...
export const ImageModal = (props: Props): JSX.Element => {
  ...
  // Mobile view has the close button and the arrow buttons are in the bottom
  if (isMobile) {
    return (
      <div
        style={{
        ...
        >
          {
            !isFirst &&
            <div
              onClick={(e) => { !isFirst && setIndex(index - 1); e.stopPropagation(); }}
              style={commonButtonProps}
            >
              { leftArrowIcon }
            </div>
          }
          {
            !isLast &&
            <div
              onClick={(e) => { setIndex(index + 1); e.stopPropagation(); }}
              style={commonButtonProps}
            >
              { rightArrowIcon }
            </div>
